...
async function submitForm() {
  ...
  if (asiste === 'si') {
    ...
    // Un recuento por plato, en el mismo orden que PLATOS
    const conteo = PLATOS.map(p => seleccion.filter(v => v === p.nombre).length);
    nCordero  = conteo[0];
    nTernera  = conteo[1];
    nEspecial = seleccion.filter(v => !v).length; // vegetariano, vegano o infantil

    platosResumen = PLATOS
      .map((p, i) => ({ corto: p.corto, n: conteo[i] }))
      .filter(x => x.n > 0)
      .map(x => `${x.corto} ×${x.n}`)
      .join(' · ');

    if (nEspecial > 0) {
      platosResumen += `${platosResumen ? ' · ' : ''}Menú especial ×${nEspecial}`;
    }
  }
