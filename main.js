...
(function initIntroScreen() {
  ...
  function openEnvelope() {
    ...
    // 1. Ocultar instrucción
    introHint.classList.add('hidden');

    // 2. Detener flotación → vibración (romper el lacre)
    stage.classList.add('no-float');

    // 3. Sello se rompe y desaparece
    setTimeout(() => seal.classList.add('breaking'), 220);

    // 4. Solapa abre en 3D + permitir overflow para la carta
    setTimeout(() => {
      wrapper.classList.add('open');
      flapWrap.classList.add('opening');
    }, 480);

    // 5. Luz interior aparece mientras la solapa se abre
    setTimeout(() => innerGlow.classList.add('glowing'), 920);

    // 6. La carta sube con efecto de brillo
    setTimeout(() => letterCard.classList.add('rising'), 1080);

    // 7. Transición cinemática hacia la web
    setTimeout(() => introScreen.classList.add('dismissing'), 2700);
